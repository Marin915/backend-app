...
public ResponseEntity<?> registrarSalidaCasa(
  ...
  @PostMapping("/crear-casa")
    ...
    return ResponseEntity.ok(toCasaDTO(guardada));
}
...
public ResponseEntity<List<MaterialCasaDTO>> obtenerMaterialesPorCasa(@PathVariable Long id) {
 ...
 @GetMapping("/materiales/entregas")
    public ResponseEntity<List<MaterialEntregaDTO>> obtenerEntregas() {
        return ResponseEntity.ok(
            casaService.obtenerEntregas()
        );
    }
